  ...
  <view class="bjx-form-item" :style="{width: theWidth}">
    <view :class="'label-' + theLabelType" :style="{alignItems: theVerticalAlign}">
      <view class="item-label" :style="theLabelStyle">
        <view v-show="thePromptType&&(thePromptType==2||theRequired)" class="item-required" :style="{color: theForm.msgColor,opacity:thePromptType&&theRequired?1:0}">
          {{ theForm.prompt }}
        </view>
        <view class="label-con">
          <slot name="label">
            ...
            <text v-if="theLabelType=='block'&&labelRight" class="right">
              {{ labelRight }}
            </text>
          </slot>
        </view>
    ...
    <scroll-view v-if="theForm.msgType=='in'" scroll-x="true" class="item-msg">
      <view :style="{color: theForm.msgColor}">
        {{ msg }}
      </view>
    </scroll-view>
  </view>
